...
function isValidIndianRegistration(value: string) {
...
export default function AddVehiclePage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const cleanedRegistration = normalizeRegistrationNumber(form.registration_number)
    ...
    if (!isValidIndianRegistration(cleanedRegistration)) {
      setError('Enter a valid registration number, example: AP12FE9073')
      return
    }

    if (!form.oem_id) {
      setError('Please select a brand / OEM')
      return
    }
    ...
    const { error: err } = await apiPost('/api/customer-vehicles', {
      ...
      registrationNumber: cleanedRegistration,
      registrationNo: cleanedRegistration,
      registration_no: cleanedRegistration,
      oemId: form.oem_id,
      modelId: form.model_id,
      manufacturingYear: form.manufacturing_year,
      manufactureYear: form.manufacturing_year,
          ...
          <Row>
            <Field label="Nickname (optional)">
              <input
                ...
                placeholder="My Splendor, Wifey's Scooty..."
              />
            </Field>
            ...
            <Field label="Registration Number *">
              <input
                ...
                onChange={e => set('registration_number', normalizeRegistrationNumber(e.target.value))}
                placeholder="AP12FE9073"
                maxLength={10}
                required
              />
              <span style={s.helpText}>Example: AP12FE9073, TS09AB1234</span>
            </Field>
          </Row>
          ...
          <Row>
            <Field label="Brand / OEM">
              <select
                ...
                value={form.oem_id}
                onChange={e => {
                  set('oem_id', e.target.value)
                  set('model_id', '')
                }}
                disabled={loadingOems || oems.length === 0}
              >
                <option value="">
                  {loadingOems ? 'Loading brands...' : oems.length ? 'Select Brand' : 'No active brands available'}
                </option>
            ...
            <Field label="Model">
              <select
                ...
                disabled={!form.oem_id || loadingModels || models.length === 0}
              >
                <option value="">
                  {!form.oem_id
                    ? 'Select brand first'
          ...
          <Row>
            <Field label="Year of Manufacture">
              <select
                ...
                onChange={e => set('manufacturing_year', parseInt(e.target.value))}
              >
                {years.map(y => (
          ...
          <Row>
            ...
            <Field label="Current Odometer (km)">
              <input
                ...
                onChange={e => set('odometer_km', parseInt(e.target.value) || 0)}
              />
            </Field>
          </Row>
          ...
          <Row>
            ...
            <Field label="AMC Expiry">
              <input
                ...
                onChange={e => set('amc_expiry', e.target.value)}
              />
            </Field>
          </Row>
        </Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      <h3 style={{ fontSize: '15px', fontWeight: '700', color: '#0f2044', margin: 0, paddingBottom: '10px', borderBottom: '1px solid #f0f2f8' }}>
        {title}
      </h3>
      {children}
    </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  helpText: { fontSize: '11px', color: '#9aa3b8' },
