...
interface Booking {
  ...
  service_centers?: { name: string; city: string }
  customer_vehicles?: { nickname: string; registration_number: string }
}
...
export default function CustomerDashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      // TODO: Confirm backend endpoints for customer dashboard aggregates.
      ...
      {activeBooking && (
        <Link to={`/my/bookings/${activeBooking.id}`} style={s.activeBooking}>
          ...
          <div style={s.activeInfo}>
            <span style={s.activeTitle}>{activeBooking.service_category}</span>
            <span style={s.activeSub}>{activeBooking.service_centers?.name} · {new Date(activeBooking.scheduled_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</span>
          </div>
              ...
              {vehicles.map(v => (
                <Link key={v.id} to={`/my/garage/${v.id}`} style={s.vehicleCard}>
                  ...
                  <div style={s.vehicleInfo}>
                    <span style={s.vehicleName}>{v.nickname || (v.vehicle_oems as {name?:string})?.name || 'Vehicle'}</span>
                    <span style={s.vehicleReg}>{v.registration_number} · {v.manufacturing_year}</span>
                    {v.insurance_expiry && isExpiringSoon(v.insurance_expiry) && (
              ...
              {bookings.slice(0, 4).map(b => (
                <Link key={b.id} to={`/my/bookings/${b.id}`} style={s.bookingCard}>
                  ...
                  <div style={s.bookingInfo}>
                    <span style={s.bookingCat}>{b.service_category}</span>
                    <span style={s.bookingMeta}>
                      <Clock size={11} /> {new Date(b.scheduled_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })} · {b.scheduled_time}
                    </span>
                    {b.service_centers && <span style={s.bookingCenter}><MapPin size={11} /> {(b.service_centers as {name?:string})?.name}</span>}
                  </div>
