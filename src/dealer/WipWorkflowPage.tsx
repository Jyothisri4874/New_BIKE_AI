...
export default function WipWorkflowPage() {
  ...
  const metrics = useMemo(() => ({
    active: records.filter(item => item.current_status !== 'delivered').length,
    pendingApproval: records.filter(item => item.approval_state === 'pending').length,
    ready: records.filter(item => item.current_status === 'ready').length,
    delivered: records.filter(item => item.current_status === 'delivered').length,
  }), [records])
  ...
  const createRecord = async (event: FormEvent) => {
    ...
    setSuccess('')
    try {
      const created = await createWipTrackingRecord({
        service_center_id: center.id,
        dealer_dms_job_no: recordForm.dealer_dms_job_no,
        customer_name: recordForm.customer_name,
        customer_phone: recordForm.customer_phone,
        customer_whatsapp: recordForm.customer_whatsapp,
        vehicle_registration_no: recordForm.vehicle_registration_no,
        vehicle_model: recordForm.vehicle_model,
        odometer_km: recordForm.odometer_km ? Number(recordForm.odometer_km) : null,
        promised_at: recordForm.promised_at ? new Date(recordForm.promised_at).toISOString() : null,
        notes: recordForm.notes,
      })
      setRecordForm(emptyRecordForm)
      setSelectedId(created.id)
      setSuccess(`Tracking record created for DMS job ${created.dealer_dms_job_no}.`)
      await loadData(center.id)
    } catch (e) {
      setError((e as Error).message || 'Could not create WIP tracking record.')
    }
    setSaving(false)
  }
    ...
    <div style={s.root}>
      ...
      <div style={s.mainGrid}>
        <section style={s.panel}>
          <div style={s.panelHead}>
            <div>
              <h2 style={s.panelTitle}><Plus size={16} /> New Tracking Record</h2>
              <p style={s.panelHint}>Official job card stays in dealer DMS.</p>
            </div>
          </div>
          <form onSubmit={createRecord} style={s.formGrid}>
            <Field label="Dealer DMS Job No" value={recordForm.dealer_dms_job_no} required onChange={value => setRecordForm({ ...recordForm, dealer_dms_job_no: value })} />
            <Field label="Customer Name" value={recordForm.customer_name} required onChange={value => setRecordForm({ ...recordForm, customer_name: value })} />
            <Field label="Customer Phone" value={recordForm.customer_phone} required onChange={value => setRecordForm({ ...recordForm, customer_phone: value })} />
            <Field label="WhatsApp" value={recordForm.customer_whatsapp} onChange={value => setRecordForm({ ...recordForm, customer_whatsapp: value })} />
            <Field label="Vehicle Reg No" value={recordForm.vehicle_registration_no} required onChange={value => setRecordForm({ ...recordForm, vehicle_registration_no: value })} />
            <Field label="Vehicle Model" value={recordForm.vehicle_model} onChange={value => setRecordForm({ ...recordForm, vehicle_model: value })} />
            <Field label="Odometer KM" value={recordForm.odometer_km} type="number" onChange={value => setRecordForm({ ...recordForm, odometer_km: value })} />
            <Field label="Promised Time" value={recordForm.promised_at} type="datetime-local" onChange={value => setRecordForm({ ...recordForm, promised_at: value })} />
            <label style={s.fullField}>
              Intake Notes
              <textarea value={recordForm.notes} onChange={event => setRecordForm({ ...recordForm, notes: event.target.value })} style={s.textarea} />
            </label>
            <button style={s.primaryBtn} disabled={saving}>
              <Save size={14} /> Create Tracking
            </button>
          </form>
        </section>
        ...
        <section style={s.panel}>
          <div style={s.panelHead}>
            <div>
              ...
              <p style={s.panelHint}>{filteredRecords.length} shown</p>
            </div>
          </div>
          ...
          <div style={s.recordList}>
            ...
            {!filteredRecords.length && <div style={s.emptyInline}>No WIP tracking records found.</div>}
          </div>
        </section>
      </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  mainGrid: { display: 'grid', gridTemplateColumns: 'minmax(320px, 0.95fr) minmax(360px, 1.05fr)', gap: '16px', alignItems: 'start' },
